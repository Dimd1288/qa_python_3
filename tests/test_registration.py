...
from constants import LOGIN_URL, URL, PROFILE_URL


class TestUserRegister:
    def test_register_of_user_is_successful(self, driver):
        user_email = utils.generate_email()
        user_password = utils.generate_password(8)
        ...
        reg_form.set_user_email(user_email, driver)
        reg_form.set_user_password(user_password, driver)
        reg_form.submit_form(driver)
        utils.wait_url_changed(driver, LOGIN_URL)
        auth_form.set_user_email(user_email, driver)
        auth_form.set_user_password(user_password, driver)
        reg_form.submit_form(driver)
        utils.wait_url_changed(driver, URL)
        Header().go_account_header_link(driver)
        utils.wait_url_changed(driver, PROFILE_URL)
        assert ProfilePage().get_user_login(driver) == user_email
